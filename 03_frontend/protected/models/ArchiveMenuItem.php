...
class ArchiveMenuItem extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            array('site_id, menu_id, parent_item_id, title', 'required'),
            array('site_id, menu_id, parent_item_id, is_external, sort_idx', 'numerical', 'integerOnly' => true),
            array('title, menu_item_abbr_cd', 'length', 'max' => 128),
            array('sort_idx, created, modified', 'safe'),
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            array('id, site_id, menu_id, parent_item_id, title, menu_item_abbr_cd, is_external, sort_idx, created, modified', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function getChildItems()
    {
        if ($this->id) {
            $criteria = new CDbCriteria();
            $criteria->order = "sort_idx ASC";
            $criteria->condition = "parent_item_id =:parent_item_id";
            $criteria->params = array(':parent_item_id' => $this->id);
